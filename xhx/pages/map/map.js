...
//深圳同方信息港经纬度，方便模拟
const FAKELAT = 39.959013
const FAKELNG = 116.351791
//地图比例尺
...
Page({
  data: {
    ...
    markers: [],
    polyline: [],
    controls: [mapControls.currentPositionControl, mapControls.carControl, mapControls.refreshControl, mapControls.personalControl ],
    cardStyle: {
      opacity: 1,
      display: 'block'
    },
  ...
  onShow() {
    app.globalData.userInfo.throttle = throttle(this.getnearPark)
    app.globalData.userInfo.userId = wx.getStorageSync('userId')
    console.log(app.globalData.userInfo.userId)
    app.globalData.userInfo.unionId = wx.getStorageSync('unionId')
    app.globalData.userInfo.openId = wx.getStorageSync('openId')
    app.globalData.userInfo.userPhone = wx.getStorageSync('userPhone')
    let _this = this;
    _this.getnearPark();
    ...
    wx.getLocation({
      ...
      success: function(res) {
        ...
        if (latitude && longitude) {
          //这里是为了测试使用，电脑定位不准确
          _this.setData({
            currentLat: test ? data.latitude : latitude,
            currentLng: test ? data.longitude : longitude,
            mapLat: test ? data.latitude : latitude,
            mapLng: test ? data.longitude : longitude
          })
          _this.setData({
            mapScale: 17
          })
  ...
  setMarkers() {
    ...
    mapControls.setPolyline(startLat, startLng, nowLat, nowLng, _this);
    _this.setData({
      markers: markers
    })
  },
    ...
    if (e.type === 'begin') {} else if (e.type === 'end') {
      mapCtx.getCenterLocation({
        success: function(res) {
          ...
          //获取当前位置之后需要重新进行数据刷新
          app.globalData.userInfo.throttle()
          //_this.getnearPark()
        }
      })
    }
  ...
  controltap(e) {
    ...
    switch (e.controlId) {
      ...
      // 价格
        _this.setData({
          markerType: 1,
          controls: [mapControls.currentPositionControl, mapControls.priceControl, mapControls.refreshControl, mapControls.personalControl]
        });
        _this.setMarkers();
        break;
      case 5:
