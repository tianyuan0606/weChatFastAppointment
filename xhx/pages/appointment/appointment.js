const app = getApp()
import { filteTime, disposeTime, dataChange } from '../../utils/disposetime.js';
...
Page({
  ...
  addCar() {
    wx.navigateTo({
      url: '../../pages/platelist/platelist?entrance=appointment'
    })
  },
  /**
   * 获取当前的系统时间
  ...
  */
  ...
  bookingSpace(e) {
    ...
    let data = Object.assign({}, that.data.params, {
      user_id: wx.getStorageSync('userId'),
      parklot_id: app.globalData.parkingInfo.parkingId, plate_id: that.data.plateId
    });
    if (app.globalData.optionType == 'fastparkinglock') {
      data.parkloc_id = app.globalData.parkingInfo.parklocId;
    } else if (app.globalData.optionType == 'fastparkingarea') {
